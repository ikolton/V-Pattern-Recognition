...
from VisualSearch.model.VSM import VSMForCausalLMNanoOwl
...
def nanoowl_args():
	...
	parser_nanoowl.add_argument("--image_encoder_engine", type=str,
								default="nanoowl/data/owl_image_encoder_patch32.engine")
	return parser_nanoowl.parse_args()
...
class VSM:
	def __init__(self, args, nanoowl_args=None):
		kwargs = {}
		...
		vsm_model = VSMForCausalLMNanoOwl.from_pretrained(
				args.version, nanoowl_args=nanoowl_args, low_cpu_mem_usage=True, vision_tower=args.vision_tower, loc_token_idx=loc_token_idx, **kwargs
			)
	...
	def inference(self, image, question=None, objects_to_locate=None, mode='segmentation'):
		# * Preparing image
		background_color = tuple(int(x * 255) for x in self.clip_image_processor.image_mean)
		image_clip = self.clip_image_processor.preprocess(expand2square(image, background_color), return_tensors="pt")[
		...
		if mode == 'segmentation':
			...
			pred_mask = pred_masks[0]
			pred_mask = torch.clamp(pred_mask, min=0)
			return (det_result['pred_boxes'][0].cpu(),
					det_result['scores'][0].cpu(),
					pred_mask[-1])
...
def visual_search_queue(vsm, image, target_object_name, current_patch, search_path, queue,  smallest_size=224, confidence_high=0.5, target_cue_threshold=6.0, target_cue_threshold_decay=0.7, target_cue_threshold_minimum=3.0):
	...
	pred_bboxes, scores, target_cue_heatmap = vsm.inference(copy.deepcopy(image_patch), question=question, objects_to_locate=[target_object_name], mode='detection')
	if len(scores) > 0:
		top_index = scores.view(-1).argmax()
		top_score = scores.view(-1).max()
		final_bbox = pred_bboxes[top_index].view(4)
		final_bbox = final_bbox * torch.Tensor([1, 1, 1, 1])
		final_bbox[2] = final_bbox[2] - final_bbox[0]
		final_bbox[3] = final_bbox[3] - final_bbox[1]
		if top_score > confidence_high:
			...
			if len(search_path) == 1:
				all_valid_boxes = pred_bboxes[scores.view(-1) > 0.5].view(-1, 4)
				all_valid_boxes = all_valid_boxes * torch.Tensor([[1, 1, 1, 1]])
				all_valid_boxes[:, 2] = all_valid_boxes[:, 2] - all_valid_boxes[:, 0]
				all_valid_boxes[:, 3] = all_valid_boxes[:, 3] - all_valid_boxes[:, 1]
				return True, search_path, all_valid_boxes
			return True, search_path, None
		else:
			search_path[-1]['temp_detection_result'] = (top_score, final_bbox)
	...
	if not search_successful:
		...
		for i, search_step in enumerate(search_path):
			if 'temp_detection_result' in search_step:
				if search_step['temp_detection_result'][0] > max_score:
					max_score = search_step['temp_detection_result'][0]
					final_step = search_step
					path_length = i+1
		# if final_step is not None and 'temp_detection_result' in final_step:
		final_step['detection_result'] = final_step['temp_detection_result'][1]
		if max_score >= confidence_low:
...
def main(args):
	...
	vsm = VSM(args, nanoowl_args=nanoowl_args())
