...
class VSMMetaModelNanoOwl:
	def __init__(
		self,
		config,
		**kwargs,
	):
		super(VSMMetaModelNanoOwl, self).__init__(config)

		self.config = config
		self.nanoowl_args = kwargs["nanoowl_args"]
		if not hasattr(self.config, "train_mask_decoder"):
			self.config.train_mask_decoder = kwargs["train_mask_decoder"]
			self.config.out_dim = kwargs["out_dim"]
		else:
			is_eval = kwargs.get('is_eval', False)
			self.initialize_lisa_modules(self.nanoowl_args, self.config, is_eval)  # * nanoowl_args

	def initialize_lisa_modules(self, args, config, is_eval=False):
		# NanoOwl
		self.nanoowl = OwlPredictor(
			self.nanoowl_args.model,
        	image_encoder_engine=args.image_encoder_engine)
		self.nanoowl.train()
		# ! To trzeba ustawić później
		for param in self.nanoowl.model.owlvit.parameters():
			param.requires_grad = True

		for param in self.nanoowl.model.owlvit.vision_model.parameters():
			param.requires_grad = False
		self.nanoowl.model.owlvit.vision_model.eval()

		# for param in self.nanoowl.model.owlvit.box_head.parameters():
		# 	param.requires_grad = False

		self.visual_projection = nn.Linear(self.nanoowl.model.owlvit.vision_model.config.hidden_size, 256, bias=False)
		self.visual_projection = self.visual_projection.bfloat16() # ? Some other way?
		for param in self.visual_projection.parameters():
			param.requires_grad = True
...
class VSMModelNanoOwl(VSMMetaModelNanoOwl, LlavaLlamaModel):
	def __init__(
		self,
		config,
		**kwargs,
	):
		super(VSMModelNanoOwl, self).__init__(config, **kwargs)
...
class VSMForCausalLMNanoOwl(LlavaLlamaForCausalLM):
	def __init__(
			self,
			config,
			**kwargs,
		...
		self.model = VSMModelNanoOwl(config, **kwargs)
	...
	def get_visual_embs(self, pixel_values: torch.FloatTensor):
		with torch.no_grad():
			encoded_output = self.model.nanoowl.encode_image(pixel_values)
			image_embeds = encoded_output.image_embeds
	...
	def model_forward(
		...
		for i in range(len(pred_embeddings_det)):
			bs = pred_embeddings_det[i].shape[0]
			detection_result = self.model.owlvit(image_embeddings[i].unsqueeze(0).repeat(bs, 1, 1, 1),
												 pred_embeddings_det[i].unsqueeze(1))
			detection_result_batch.append(detection_result)
		...
		for bboxes_label, bboxes_valid in zip(bboxes_labels_list, bboxes_valid_list):
			target_det.append(
				{"labels": torch.zeros(len(bboxes_label)).to(bboxes_label.device, torch.long), "boxes": bboxes_label})
			if bboxes_valid:
				all_bboxes_valid.append(
					torch.ones((min(24 * 24, len(bboxes_label)), 1)).to(bboxes_label.device, torch.long))
			else:
				all_bboxes_valid.append(
					torch.zeros((min(24 * 24, len(bboxes_label)), 1)).to(bboxes_label.device, torch.long))
		all_bboxes_valid = torch.cat(all_bboxes_valid, 0)

		loss_dict = self.model.owlvit.criterion(detection_result_batch, target_det, num_boxes) # ! Nanoowl
	...
	def inference(
		...
		with torch.no_grad():
			...
			assert len(self.model.text_hidden_fcs_det) == 1
			hidden_states_det.append(self.model.text_hidden_fcs_det[0](
				output_hidden_states))  # ! output_hidden_states is most likely a <LOC>?
			last_hidden_state_det = torch.stack(hidden_states_det, dim=-1).sum(dim=-1)
			...
			for i in range(len(pred_embeddings_seg)):
				...
				sparse_embeddings = sparse_embeddings.to(pred_embeddings_seg[i].dtype)
				low_res_masks, iou_predictions = self.model.mask_decoder(
					image_embeddings=self.model.visual_projection(image_embeddings[i].unsqueeze(0)).permute(0, 3, 1, 2), # 24
					image_pe=self.model.prompt_encoder.get_dense_pe(), # 48
					sparse_prompt_embeddings=sparse_embeddings,
					dense_prompt_embeddings=dense_embeddings, # 48
					multimask_output=multimask_output,
				)
				pred_mask = F.interpolate(
					low_res_masks.float(), original_size_list[i], mode="bilinear", align_corners=False
			...
			# ! We need to put object into [] and separate them with commas
			objects_to_locate_text = f"[{', '.join(objects_to_locate)}]"

			text_encodings = self.model.nanoowl.encode_text(objects_to_locate_text)

			output = self.model.nanoowl.predict(
				image=original_image,
				text=objects_to_locate_text,
				text_encodings=text_encodings,
				threshold=0.0,
				pad_square=False
			)

			pred_boxes = output.boxes.unsqueeze(0)
			scores = output.scores.unsqueeze(0)
			detection_result_batch = {'scores': scores, 'pred_boxes': pred_boxes}
