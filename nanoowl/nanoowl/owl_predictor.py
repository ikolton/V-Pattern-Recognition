...
class OwlPredictor(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.device = device
        self.model = OwlViTForObjectDetection.from_pretrained(model_name, low_cpu_mem_usage=True, torch_dtype=torch.bfloat16).to(self.device).eval()
        self.processor = OwlViTProcessor.from_pretrained(model_name, low_cpu_mem_usage=True, torch_dtype=torch.bfloat16)
    ...
    def decode(self, 
        ...
        image_class_embeds = image_class_embeds.bfloat16()
        query_embeds = query_embeds.bfloat16()
